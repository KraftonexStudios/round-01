...
const SideFrontView = () => {
  ...
  useGSAP(() => {
    const Begin_text = new SplitType(Beginningref.current, { types: "chars" });
    sidetl.current = gsap
      .timeline({
        scrollTrigger: {
          trigger: middleRef.current,
          // markers: true,
          start: () => (window.innerWidth < 600 ? "50% top" : "110% top"),
          end: () => (window.innerWidth < 600 ? "150% top" : "240% top"),
          scrub: true,
        },
      })
      .to("#svg", {
  ...
  return (
    <div ref={middleRef} className="side-view-container">
      <div>
        <svg
          className="svg z-[-2]"
          width="200"
      ...
      <div className="side-text-container">
        <h1 className="">THE</h1>
        <div className="side-view-text1">
          <h3 className="relative z-10 whitespace-nowrap" ref={Beginningref}>
            BEGINNING
          </h3>
        </div>
      ...
      <div className="con-range2">
        <img className="" src="../../Images/range2.png" alt="RANGE ROVER" />
      </div>
      <div>
        <svg
